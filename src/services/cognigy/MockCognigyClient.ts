...
export class MockCognigyClient implements CognigyClient {
  ...
  async sendInfo(info: CognigyInfo) {
    ...
    try { await fetch('/api/cognigy/action', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(info) }); } catch { /* A demonstração continua local caso a API não esteja em execução. */ }
  }
  async startCall() { this.emitState('connecting'); await new Promise(r => setTimeout(r, 500)); this.emitState('active'); this.emitTranscript({ id: crypto.randomUUID(), author:'agent', text:'Olá! Sou a Carla, sua especialista em atendimento bancário. Como posso ajudar?', time:new Date().toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}) }); }
