...
interface BankingValue {
  ...
  cardDetail: CardId | null;
  selectedMachine: string;
  selectCard: (id: CardId) => void;
  showSection: (section: string) => void;
  openComparison: (a: CardId, b: CardId) => void;
  closeComparison: () => void;
  openCardDetail: (id: CardId) => void;
...
export function BankingProvider({ children }: { children: ReactNode }) {
  const [selectedCard, setSelectedCard] = useState<CardId>(cards[0].id);
  const [section, setSection] = useState('overview');
  const [highlight, setHighlight] = useState<string | null>(null);
  const [comparison, setComparison] = useState<Comparison | null>(null);
  const [cardDetail, setCardDetail] = useState<CardId | null>(null);
  const [selectedMachine, setSelectedMachine] = useState('Todas as maquininhas');

  const showSection = (value: string) => {
    setSection(value);
    setHighlight(value);
    window.setTimeout(() => setHighlight(null), 4200);
  };
  ...
  return (
    <BankingContext.Provider
      value={{
        ...
        showSection,
        openComparison: (cardA, cardB) => setComparison({ cardA, cardB }),
        closeComparison: () => setComparison(null),
        openCardDetail: (id) => {
          setSelectedCard(id);
          setCardDetail(id);
        },
        closeCardDetail: () => setCardDetail(null),
...
export const useBanking = () => {
  ...
  if (!value) throw new Error('useBanking deve estar dentro de BankingProvider');
  return value;
};
